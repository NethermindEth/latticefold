...
fn test_construct_polynomial() {
    ...
    let (_, g_degree, mz_mles) =
        LFLinearizationProver::<RqNTT, PoseidonTranscript<RqNTT>>::construct_polynomial_g(
            &z_ccs,
            &mut transcript,
            &ccs,
        )
...
fn test_generate_sumcheck() {
    ...
    let (_, point_r) =
        LFLinearizationProver::<RqNTT, PoseidonTranscript<RqNTT>>::generate_sumcheck_proof(
            &mut transcript,
            g_mles,
            ccs.s,
            g_degree,
...
fn prepare_test_vectors<RqNTT: SuitableRing>(
    wit: &Witness<RqNTT>,
    cm_i: &CCCS<RqNTT>,
    ccs: &CCS<RqNTT>,
...
fn test_compute_v() {
    ...
    let (wit, cm_i, ccs, _) = setup_test_environment::<RqNTT, DP>(None, n);
    let (point_r, Mz_mles) = prepare_test_vectors::<RqNTT>(&wit, &cm_i, &ccs);

    // Compute actual v vector
    let (point_r, v, _) =
        LFLinearizationProver::<RqNTT, PoseidonTranscript<RqNTT>>::compute_evaluation_vectors(
            &wit, &point_r, &Mz_mles,
        )
        .unwrap();
...
fn test_full_prove() {
    ...
    let (lcccs, proof) = LFLinearizationProver::<RqNTT, PoseidonTranscript<RqNTT>>::prove(
        &cm_i,
        &wit,
        &mut transcript,
        &ccs,
...
fn test_verify_sumcheck_proof() {
    ...
    let n = WIT_LEN * DP::L;
    let (wit, cm_i, ccs, _) = setup_test_environment::<RqNTT, DP>(None, n);
    let mut prove_transcript = PoseidonTranscript::<RqNTT>::default();
    ...
    let mut verify_transcript = PoseidonTranscript::<RqNTT>::default();

    // Generate beta challenges to match prover's transcript state
    let _ = verify_transcript.squeeze_beta_challenges(ccs.s);

    let result = LFLinearizationVerifier::<RqNTT, PoseidonTranscript<RqNTT>>::verify_sumcheck_proof(
        &proof,
...
fn test_verify_evaluation_claim() {
    ...
    // Generate proof
    let (_, proof) = LFLinearizationProver::<RqNTT, PoseidonTranscript<RqNTT>>::prove(
        &cm_i,
        &wit,
        &mut transcript,
        &ccs,
    ...
    let (point_r, s) =
        LFLinearizationVerifier::<RqNTT, PoseidonTranscript<RqNTT>>::verify_sumcheck_proof(
            &proof,
            &mut transcript,
            &ccs,
        )
    ...
    let result =
        LFLinearizationVerifier::<RqNTT, PoseidonTranscript<RqNTT>>::verify_evaluation_claim(
...
fn test_prepare_verifier_output() {
    ...
    let lcccs =
        LFLinearizationVerifier::<RqNTT, PoseidonTranscript<RqNTT>>::prepare_verifier_output(
            &cm_i,
            point_r.clone(),
            &proof,
        );
...
fn test_verify_invalid_proof() {
    ...
    let (_, mut proof) = LFLinearizationProver::<RqNTT, PoseidonTranscript<RqNTT>>::prove(
        &cm_i,
        &wit,
        &mut transcript,
        &ccs,
    ...
    let result = LFLinearizationVerifier::<RqNTT, PoseidonTranscript<RqNTT>>::verify(
        &cm_i,
        &proof,
        &mut transcript,
        &ccs,
